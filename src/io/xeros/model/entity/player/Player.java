...
import io.xeros.content.advancedslayer.Difficulty;
import io.xeros.content.advancedslayer.Gear;
...
import io.xeros.content.deals.CosmeticDeals;
import io.xeros.content.deals.TimeOffers;
import io.xeros.content.deathstorage.DeathInterface;
import io.xeros.content.dialogue.DialogueBuilder;
import io.xeros.content.dialogue.DialogueOption;
import io.xeros.content.donationrewards.DonationRewards;
import io.xeros.content.donor.CosmeticManager;
import io.xeros.content.donorpet.PetManagement;
...
import io.xeros.content.fusion.FusionSystem;
...
import io.xeros.content.minigames.rangingguild.RangingGuild;
import io.xeros.content.minigames.TOA.TombsOfAmascutContainer;
...
import io.xeros.content.minigames.wheel.WheelOfFortune;
...
import io.xeros.content.taskmaster.TaskMaster;
...
import io.xeros.content.tradingpost.POSManager;
import io.xeros.content.tradingpost.TradePostOffer;
...
import io.xeros.model.entity.player.mode.*;
...
public class Player extends Entity {
    ...
    public final int[] positions = new int[]{0};
...
//        this.tempEquipmentCosmetic = new int[14];
    ...
    public boolean isAggressionTimeout(Player player) {
        if (Boundary.isIn(player, Boundary.GODWARS_AREA) || Boundary.isIn(player, Boundary.CERBERUS_BOSSROOMS))
            return false;
        if (Boundary.isIn(player, ArbograveConstants.ALL_BOUNDARIES)) return false;
        if (player.usingInfAgro) return false;
    ...
    private io.xeros.content.dwarfleaguecannon.Cannon dwarfCannon;
    ...
    public Player(Channel channel) {
        ...
        for (int i = 0; i < playerItems.length; i++) playerItems[i] = 0;
        for (int i = 0; i < playerItemsN.length; i++) playerItemsN[i] = 0;
        resetSkills();
    ...
    public void resetSkills() {
        for (int i = 0; i < playerLevel.length; i++)
            if (i == 3) playerLevel[i] = 10;
            else playerLevel[i] = 1;
        for (int i = 0; i < playerXP.length; i++)
            if (i == 3) playerXP[i] = 1300;
            else playerXP[i] = 0;
    }
    ...
    public void setAddressChanged(String type, String previous, String current, boolean staffAlertMessage) {
        addQueuedLoginAction(plr -> {
            ...
            if (staffAlertMessage) Discord.writeAddressSwapMessage(message);
            else Discord.writeServerSyncMessage(message);
    ...
    public long getTotalXp() {
        long temp = 0;
        for (int i = 0; i < playerXP.length; i++) temp += playerXP[i];
        return temp;
    }

    public boolean viewable(NPC npc, boolean updatingLocalList) {
        // Only check against these when updating local npc list, not when adding to local list
        if (updatingLocalList) if (npc.teleporting) return false;
    ...
    private void loadController() {
        ...
        else setController(ControllerRepository.getOrDefault(this));
    ...
    public void updateController() {
        if (getController().allowSwitch()) {
            ...
            if (newController != controller) setController(newController);
        }
    }
    ...
    public void climbLadderTo(Position position, Consumer<Player> finishConsumer) {
        ...
        setTickable((container, player) -> {
            if (container.getTicks() == 1) {
                ...
                if (finishConsumer != null) finishConsumer.accept(this);
            }
        });
    }
    ...
    public void destruct() {
        ...
        if (party != null) party.remove(this);
        if (getPA().viewingOtherBank) getPA().resetOtherBank();
        if (this.clan != null) this.clan.removeMember(this);
        ...
        if (Configuration.BOUNTY_HUNTER_ACTIVE) if (getBH().hasTarget()) getBH().setWarnings(getBH().getWarnings() + 1);
        ...
        if (getCannon() != null) getCannon().pickup(this, false);
        if (getLeagueCannon() != null) getLeagueCannon().pickup(this, false);
        if (combatLevel >= 100) {
            if (Highpkarena.getState(this) != null) Highpkarena.removePlayer(this, true);
        } else if (combatLevel >= 80 && combatLevel <= 99)
            if (Lowpkarena.getState(this) != null) Lowpkarena.removePlayer(this, true);
        ...
        if (Boundary.isIn(this, Boundary.XERIC_LOBBY)) XericLobby.removePlayer(this);
        if (getRaidsInstance() != null) getRaidsInstance().logout(this);
        if (Vorkath.inVorkath(this)) this.getPA().movePlayer(2272, 4052, 0);
        if (Vorkath.inVorkath(this)) getPA().movePlayer(2272, 4052, 0);
        if (getPA().viewingOtherBank) getPA().resetOtherBank();
        if (Boundary.isIn(this, PestControl.GAME_BOUNDARY)) PestControl.removeGameMember(this);
        if (Boundary.isIn(this, PestControl.LOBBY_BOUNDARY)) PestControl.removeFromLobby(this);

        if (Boundary.isIn(this, new Boundary(1920, 4416, 1983, 4479))) getPA().forceMove(3288, 2786, 0, true);
...
/*        if (Discord.jda != null) {
        ...
        removeFromInstance();
        if (clan != null) clan.removeMember(this);
        inStream = null;
        //outStream = null;
        playerListSize = 0;
        npcListSize = 0;
...
//        getDiaryManager().setDiariesCompleted();
        ...
        if (Halloween.isHalloween()) Halloween.handleBuckets(this);
...
//            getPA().sendGameTimer(ClientGameTimer.RAGE_POT, TimeUnit.MINUTES, (int) MM);
        ...
        if (Christmas.isChristmas()) sendMessage("@red@Ho Ho Ho, Merry Christmas!!!");
        ...
        else if (Right.YOUTUBER.equals(getRights().getPrimary()))
            getTitles().setCurrentTitle("@bla@[<col=004080>" + "<shad=1>YouTuber</shad>" + "</col>@bla@]");

        if (bonusDmgTicks > 0) {
            if (!bonusDmg) bonusDmg = true;
        ...
        if (IslandTimer > 0) if (IslandTimer <= 15)
        ...
        if (combatLevel >= 126) getEventCalendar().progress(EventChallenge.HAVE_126_COMBAT);

        if (getRights().getPrimary().equals(Right.HELPER))
            PlayerHandler.executeGlobalMessage("<col=004080>" + getDisplayNameFormatted() + "@bla@ has just logged in!");
        else if (getRights().getPrimary().equals(Right.MODERATOR))
            PlayerHandler.executeGlobalMessage("<col=DADADA>" + getDisplayNameFormatted() + "@bla@ has just logged in!");
        else if (getRights().getPrimary().equals(Right.COMMUNITY_MANAGER))
            PlayerHandler.executeGlobalMessage("<col=27FA17>" + getDisplayNameFormatted() + "@bla@ has just logged in!");
        else if (getRights().getPrimary().equals(Right.GAME_DEVELOPER))
            PlayerHandler.executeGlobalMessage("<col=A10081>" + getDisplayNameFormatted() + " has just logged in!");
        else if (getRights().getPrimary().equals(Right.ADMINISTRATOR))
            PlayerHandler.executeGlobalMessage("<col=F5FF0F>" + getDisplayNameFormatted() + "@bla@ has just logged in!");
        else /* else if (getRights().getPrimary().equals(Right.GOGETTA_SS2)) {
            PlayerHandler.executeGlobalMessage("[@blu@Sponsor@bla@] <col=255>" + getDisplayNameFormatted() + "@bla@ has just logged in!");
        }*/ if (getRights().getPrimary().equals(Right.STAFF_MANAGER))
                PlayerHandler.executeGlobalMessage("<col=0f07eb>" + getDisplayNameFormatted() + " has just logged in!");


        CosmeticManager.onLogin(this);

        if (getSlayer().superiorSpawned) getSlayer().superiorSpawned = false;
...
//                    rights.add(right);
        ...
        if (hasFollower) if (petSummonId > 0) {
            PetHandler.Pets pet = PetHandler.forItem(petSummonId);
            if (pet != null) PetHandler.spawn(this, pet, true, false);
        }
        if (hasThrall) if (ThrallSummonId > 0) {
            ThrallSystem thrall = ThrallSystem.forThrall(ThrallSummonId);
            if (thrall != null) ThrallSystem.spawnThrall(this, thrall);
        }
        for (int m = 0; m < activeMageArena2BossId.length; m++) activeMageArena2BossId[m] = 0;
        ...
        if (getPrivateChat() > 2) setPrivateChat(0);
        if (getOutStream() != null) {
        ...
        if (totalLevel >= 2000) getEventCalendar().progress(EventChallenge.HAVE_2000_TOTAL_LEVEL);
        if (totalLevel >= 2376) Achievements.increase(this, AchievementType.MAX, 1);
        ...
        if (!completedTutorial) {
            ...
            if (mode == null) mode = Mode.forType(ModeType.STANDARD);
        ...
        addEvents();
        if (Configuration.BOUNTY_HUNTER_ACTIVE) bountyHunter.updateTargetUI();
        for (int i = 0; i < 25; i++) {
        ...
        if (pin.requiresUnlock()) pin.open(2);
        if (health.getCurrentHealth() < 10) health.setCurrentHealth(10);
        // Update experience counter on login
        int[] ids = new int[playerLevel.length];
        for (int skillId = 0; skillId < ids.length; skillId++) ids[skillId] = skillId;
        if (experienceCounter > 0L) playerAssistant.sendExperienceDrop(false, experienceCounter, ids);

        rechargeItems.onLogin();
        for (int i = 0; i < getQuick().getNormal().length; i++)
            if (getQuick().getNormal()[i]) getPA().sendConfig(QuickPrayers.CONFIG + i, 1);
            else getPA().sendConfig(QuickPrayers.CONFIG + i, 0);
        PollTab.updateInterface(this);
        if (EventCalendar.isEventRunning()) sendMessage(EventCalendar.LOGIN_MESSAGE);
        ...
        if (Gobbler.spawned) sendMessage("@cr28@[@red@Gobbler@bla@] @red@has just spawned " + Gobbler.SpawnLocation);
        ...
        if (EliteCentBoost > 0) sendErrorMessage("You have the Elite Cent Boost Active!");
        if (weeklyInfPot > 0) {
        ...
        if (dailyDamage > 0) sendErrorMessage("You have the 2x Damage Boost Active!");
        if (daily2xRaidLoot > 0) sendErrorMessage("You have the 2x Raid loot Boost Active!");
        if (daily2xXPGain > 0) sendErrorMessage("You have the 2x XP Gain Boost Active!");
        if (doubleDropRate > 0) sendErrorMessage("You have the Double Drop Rate Boost Active!");
        if (tradePost == null) tradePost = new POSManager();
        tradePost.init(this);
        ...
        if (Discord.jda != null)
            Discord.jda.getPresence()
                    .setPresence(OnlineStatus.ONLINE, Activity.playing("Turmoil with " + PlayerHandler.getPlayerCount() + " players!"));
    }

    private void decrementBoostDurations() {
        // Decrement remaining time for each active boost
        if (EliteCentBoost > 0) EliteCentBoost--;
        if (weeklyInfPot > 0) weeklyInfPot--;
        if (weeklyInfAgro > 0) weeklyInfAgro--;
        if (weeklyOverload > 0) weeklyOverload--;
        if (weeklyRage > 0) weeklyRage--;
        if (dailyDamage > 0) dailyDamage--;
        if (daily2xRaidLoot > 0) daily2xRaidLoot--;
        if (daily2xXPGain > 0) daily2xXPGain--;
        if (doubleDropRate > 0) doubleDropRate--;
    ...
    public void debug(String message) {
        if (debugMessage) sendMessage(message);
    }
    ...
    public void healEverything() {
        ...
        if (getHealth().getCurrentHealth() < getHealth().getMaximumHealth()) getHealth().reset();
    ...
    public void graceSum() {
        graceSum = 0;
        for (int grace : AgilityHandler.gracefulIds) if (getItems().isWearingItem(grace)) graceSum++;
        if (SkillcapePerks.AGILITY.isWearing(this) || SkillcapePerks.isWearingMaxCape(this)) graceSum++;
    }
    ...
    public void processQueuedActions() {
        ...
        while ((action = queuedActions.poll()) != null) action.accept(this);
    }
    ...
    private void processTickables() {
        ...
        for (TickableContainer<Player> tickable : tickablesCopy)
            if (tickable.isStopped() || !tickable.tick(this)) removeList.add(tickable);
    ...
    public TickableContainer<Player> setTickable(Tickable<Player> tickable) {
        if (this.tickable != null) this.tickable.stop();
        if (tickable != null) {
    ...
    public void raidsClipFix() {
        if (Boundary.RAIDS.in(this)) wasInRaids = true;
        else if ((wasInRaids)) {
    ...
    public void interruptActions() {
        for (int i = 0; i < playerSkilling.length; i++) playerSkilling[i] = false;
        setTickable(null);
    }

    public void process() {
        itemPickedUpThisTick = false;
        updateDiscordPoints(this);
        ...
        if (getCannon() != null) getCannon().tick(this);
        if (getLeagueCannon() != null) getLeagueCannon().tick(this);
        ...
        if (!isCompletedTutorial()
                ...
                && !isInterfaceOpen(ModeSelection.INTERFACE_ID)) modeSelection.openInterface();
        ...
        if (getOpenInterface() == 24605) TimeOffers.update(this);

        if (getOpenInterface() == 24565) CosmeticDeals.updateOffers(this);

        if (getOpenInterface() == 24505) AccountBoosts.handleTimer(this);

        if (usingInfPrayer) if (Boundary.isIn(this, Boundary.DUEL_ARENA) || Boundary.isIn(this, Boundary.WILDERNESS)) {
        ...
        if (IslandTimer > 0) {
            ...
            if (IslandTimer <= 0) {
                ...
                if (Boundary.isIn(this, Boundary.UNICOW_AREA)) moveToHome();
            }
        }
        if (SafetyTimer > 0) {
            SafetyTimer--;
        ...
        if (getInstance() != null) getInstance().tick(this);
        if (isRunningToggled() && runEnergy <= 0) updateRunningToggled(false);
        if (staminaDelay > 0) staminaDelay--;
        if (gwdAltarTimer > 0) gwdAltarTimer--;
        if (gwdAltarTimer == 1) sendMessage("You can now operate the godwars prayer altar again.");
        if (updateItems) {
            itemAssistant.updateItems();
            updateItems = false;
        }
        if (bonusXpTime > 0) bonusXpTime--;
        if (bonusXpTime == 1) sendMessage("@blu@Your time is up. Your XP is no longer boosted by the voting reward.");
        if (isDead && respawnTimer == -6) PlayerDeath.applyDead(this);
        if (respawnTimer == 9) {
            respawnTimer = -6;
        ...
        if (underAttackByPlayer > 0) if (System.currentTimeMillis() - singleCombatDelay > inCombatDelay) {
            underAttackByPlayer = 0;
            setInCombatDelay(Configuration.IN_COMBAT_TIMER);
        }
        if (underAttackByNpc > 0) if (System.currentTimeMillis() - singleCombatDelay2 > inCombatDelay) {
            underAttackByNpc = 0;
            setInCombatDelay(Configuration.IN_COMBAT_TIMER);
        }
        if (hasOverloadBoost) if (System.currentTimeMillis() - lastOverloadBoost > 3000) {
        ...
        if (targeted != null) if (distanceToPoint(targeted.getX(), targeted.getY()) > 10) {
    ...
    public void processCombat() {
        ...
        if (npcAttackingIndex > 0 && clickNpcType == 0 || playerAttackingIndex > 0)
            if (getCombatItems().doQueuedGraniteMaulSpecials())
                return;
        if (npcAttackingIndex > 0 && clickNpcType == 0) attacking.attackEntity(NPCHandler.npcs[npcAttackingIndex]);
        if (playerAttackingIndex > 0) attacking.attackEntity(PlayerHandler.players[playerAttackingIndex]);
    }
    ...
    private void sendAreaInterfaces() {
        ...
        if (Boundary.isIn(this, FlowerPoker.BOUNDARIES)) getPA().showOption(1, 0, "<img=29>Gamble with");
        ...
        if (getPosition().inWild() && !getPosition().inClanWars()) {
            ...
            if (Configuration.SINGLE_AND_MULTI_ZONES) getPA().sendFrame126("@yel@Level: " + wildLevel, 199);
            else {
                getPA().multiWay(-1);
            ...
            // getPA().walkableInterface(197);
            wildLevel = 54;
        } else if (getItems().isWearingItem(10501, 3) && !getPosition().inWild()) getPA().showOption(3, 0, "Throw-At");
        else if (getPosition().inEdgeville()) {
            if (Configuration.BOUNTY_HUNTER_ACTIVE) if (bountyHunter.hasTarget()) {
                getPA().walkableInterface(28000);
                ...
                if (regularHelm || imbuedHelm) getSlayer().displayInterface();
                else getPA().walkableInterface(-1);
            } else getPA().walkableInterface(-1);
            else {
                getPA().sendFrame99(0);
        ...
        } else if (Boundary.isIn(this, Boundary.DONATOR_ZONE_BLOODY)) getPA().walkableInterface(35427);
        ...
        } else /* else if (getPosition().inRaidLobby() || Boundary.isIn(this, Boundary.XERIC_LOBBY)) {
            getPA().walkableInterface(6673);
        }*/ if (Boundary.isIn(this, Boundary.SKOTIZO_BOSSROOM)) getPA().walkableInterface(29230);
        else if (Boundary.isIn(this, Boundary.WINTERTODT)) getPA().walkableInterface(63000);
        else if (getSlayer().getTask().isPresent()) {
            IntPredicate hasHelmet = id -> getItems().isWearingItem(id, Player.playerHat);
            boolean regularHelm = IntStream.of(SLAYER_HELMETS).anyMatch(hasHelmet) || IntStream.of(BLACK_MASKS).anyMatch(hasHelmet);
            boolean imbuedHelm = IntStream.of(IMBUED_SLAYER_HELMETS).anyMatch(hasHelmet);
        ...
        } else if (getInstance() == null || !getInstance().handleInterfaceUpdating(this)) getPA().walkableInterface(-1);
    }
    ...
    public CollectionLog getGroupIronmanCollectionLog() {
        if (getRights().contains(Right.GROUP_IRONMAN)) {
            ...
            if (group != null && group.getCollectionLog() != null) return group.getCollectionLog();
        }
    ...
    public Skotizo getSkotizo() {
        if (getInstance() != null && getInstance() instanceof Skotizo) return (Skotizo) getInstance();
    ...
    private void processQueuedPackets(Queue<Packet> queue) {
        ...
        while ((p = queue.poll()) != null) {
            ...
            if (p.getOpcode() > 0) PacketHandler.processPacket(this, p.getOpcode(), p.getLength());
        }
    }
    ...
    public void correctCoordinates() {
        ...
        if (Boundary.isIn(new Position(teleportToX, teleportToY, heightLevel), Boundary.ZULRAH)) moveToHome();
...
//        }
    ...
    public BountyHunter getBH() {
        if (Objects.isNull(bountyHunter)) bountyHunter = new BountyHunter(this);
        return bountyHunter;
    }
    ...
    public Killstreak getKillstreak() {
        if (killstreaks == null) killstreaks = new Killstreak(this);
        return killstreaks;
    }
    ...
    public boolean insideBoundary(Boundary boundary) {
        return Boundary.isIn(this, boundary);
    }
    ...
    public boolean skillLevelGreaterThan(int skill, int level) {
        return this.getLevelForXP(this.playerXP[skill]) > level;
    }

    public boolean skillLevelGreaterThan(int skill, int level, int threshold) {
        var cached = this.getLevelForXP(this.playerXP[skill]);
        return cached >= level && cached < threshold;
    }
    ...
    public boolean maxRequirements(Player c) {
        int amount = 0;
        for (int i = 0; i <= 21; i++) {
            if (getLevelForXP(c.playerXP[i]) >= 99) amount++;
            if (amount > 21) return true;
        }
        return false;
    }
    ...
    public boolean maxedSkiller(Player c) {
        int amount = 0;
        for (int id = 0; id <= 7; id++) if (getLevelForXP(c.playerXP[id]) >= 2 && id != 3) amount++;
        for (int i = 8; i <= 22; i++) if (c.playerLevel[i] >= 99) amount++;
        return amount == 15;
    }
    ...
    public int getNextWalkingDirection() {
        ...
        do {
            ...
            if (dir == -1 && otherDirection != dir) otherDirection = dir;
    ...
    public void postProcessing() {
        if (newWalkCmdSteps > 0) {
            ...
            if (!found) println_debug("Fatal: couldn\'t find connection vertex! Dropping packet.");
                ...
                if (dir != -1 && (dir & 1) != 0) {
                    for (int i = 0; i < numTravelBackSteps - 1; i++) addToWalkingQueue(travelBackX[i], travelBackY[i]);
                ...
                } else for (int i = 0; i < numTravelBackSteps; i++) addToWalkingQueue(travelBackX[i], travelBackY[i]);
                for (int i = 0; i < newWalkCmdSteps; i++) addToWalkingQueue(getNewWalkCmdX()[i], getNewWalkCmdY()[i]);
            }
        }
    }
    ...
    public void getNextPlayerMovement() {
        ...
        if (getTeleportToX() != -1 && getTeleportToY() != -1) {
            ...
            } else runningDistanceTravelled = 0;
            int deltaX = 0;
            int deltaY = 0;
            if (currentX < 2 * 8) {
                deltaX = 4 * 8;
    ...
    public void checkLocationOnLogin() {
        if (Boundary.isIn(this, PestControl.GAME_BOUNDARY)) getPA().movePlayerUnconditionally(2657, 2639, 0);
        ...
        if (Boundary.isIn(this, Boundary.ZULRAH))
            getPA().movePlayerUnconditionally(Configuration.RESPAWN_X, Configuration.RESPAWN_Y, 0);
        for (LobbyType lobbyType : LobbyType.values())
            LobbyManager.get(lobbyType).ifPresent(lobby -> {
                if (lobby.inLobby(this)) if (lobby.canJoin(this)) lobby.attemptJoin(this);
                else getPA().movePlayerUnconditionally(3033, 6068, 0);//TODO Make this independent for all lobbies
            });
        if (Boundary.isIn(this, Boundary.RAIDS) || Boundary.isIn(this, Boundary.OLM)) RaidConstants.checkLogin(this);
    }

    public void postTeleportProcessing() {
        if (getPosition().inGodwars()) {
            if (equippedGodItems == null) updateGodItems();
    ...
    public void updatePlayerMovement(Stream str) {
        // synchronized(this) {
        if (dir1 == -1) if (isUpdateRequired() || isChatTextUpdateRequired()) {
            str.writeBits(1, 1);
            str.writeBits(2, 0);
        } else str.writeBits(1, 0);
        else if (dir2 == -1) {
    ...
    public void addNewNPC(NPC npc, Stream str, Stream updateBlock, boolean flag) {
        ...
        if (pet && npc.spawnedBy == getIndex()) str.writeBits(2, 2);
        else if (pet) str.writeBits(2, 1);
        else str.writeBits(2, 0);
    ...
    public void addNewPlayer(Player plr, Stream str, Stream updateBlock) {
        if (playerListSize >= Configuration.MAX_PLAYERS_IN_LOCAL_LIST) return;
        int id = plr.getIndex();
    ...
    protected void appendPlayerAppearance(Stream str) {
        ...
        if (titles.getCurrentTitle().equalsIgnoreCase("None")) sb.delete(0, sb.length());
        ...
        if (isNpc == false) {
            if (getEquipmentToShow(playerHat) > 1)
                appearanceUpdateBlockCache.writeUnsignedWord(512 + getEquipmentToShow(playerHat));
            else appearanceUpdateBlockCache.writeByte(0);
            if (getEquipmentToShow(playerCape) > 1)
                appearanceUpdateBlockCache.writeUnsignedWord(512 + getEquipmentToShow(playerCape));
            else appearanceUpdateBlockCache.writeByte(0);
            if (getEquipmentToShow(playerAmulet) > 1)
                appearanceUpdateBlockCache.writeUnsignedWord(512 + getEquipmentToShow(playerAmulet));
            else appearanceUpdateBlockCache.writeByte(0);
            if (getEquipmentToShow(playerWeapon) > 1)
                appearanceUpdateBlockCache.writeUnsignedWord(512 + getEquipmentToShow(playerWeapon));
            else appearanceUpdateBlockCache.writeByte(0);
            if (getEquipmentToShow(playerChest) > 1)
                appearanceUpdateBlockCache.writeUnsignedWord(512 + getEquipmentToShow(playerChest));
            else appearanceUpdateBlockCache.writeUnsignedWord(256 + playerAppearance[2]);
            if (getEquipmentToShow(playerShield) > 1 && !getItems().is2handed(ItemDef.forId(getEquipmentToShow(playerWeapon)).getName(), getEquipmentToShow(playerWeapon)))
                appearanceUpdateBlockCache.writeUnsignedWord(512 + getEquipmentToShow(playerShield));
            else appearanceUpdateBlockCache.writeByte(0);
            ...
            if (getEquipmentToShow(playerLegs) > 1)
                appearanceUpdateBlockCache.writeUnsignedWord(512 + getEquipmentToShow(playerLegs));
            else appearanceUpdateBlockCache.writeUnsignedWord(256 + playerAppearance[5]);
            if (ItemDef.forId(getEquipmentToShow(playerHat)).getEquipmentModelType() !=
                    EquipmentModelType.FULL_MASK &&
            ...
            if (getEquipmentToShow(playerHands) > 1)
                appearanceUpdateBlockCache.writeUnsignedWord(512 + getEquipmentToShow(playerHands));
            else appearanceUpdateBlockCache.writeUnsignedWord(256 + playerAppearance[4]);
            if (getEquipmentToShow(playerFeet) > 1)
                appearanceUpdateBlockCache.writeUnsignedWord(512 + getEquipmentToShow(playerFeet));
            else appearanceUpdateBlockCache.writeUnsignedWord(256 + playerAppearance[6]);
        ...
        appearanceUpdateBlockCache.writeDWord(getIndex());
        if (getMode().equals(Mode.forType(ModeType.WILDYMAN)) || getMode().equals(Mode.forType(ModeType.HARDCORE_WILDYMAN)))
            appearanceUpdateBlockCache.writeString("false");
        else appearanceUpdateBlockCache.writeString(String.valueOf(hideDonor));
        ...
        for (Right right : rightsSet) appearanceUpdateBlockCache.writeByte(right.ordinal());
    ...
    public int calculateCombatLevel() {
        ...
        if (d >= d1 && d >= d2) combatLevel += d;
        else if (d1 >= d && d1 >= d2) combatLevel += d1;
        else if (d2 >= d && d2 >= d1) combatLevel += d2;
        return combatLevel;
    }
    ...
    public int getLevel(Skill skill) {
        if (skill == Skill.DEFENCE)
            if (ToragsEffect.INSTANCE.canUseEffect(this)) return (int) ToragsEffect.modifyDefenceLevel(this);
    ...
    public void restore(Skill skill, int amount) {
        ...
        if (playerLevel[skill.getId()] > maxLevel) playerLevel[skill.getId()] = maxLevel;
        getPA().refreshSkill(skill.getId());
    }
    ...
    public void appendHeal(int amount, Hitmark h) {
        if (teleTimer <= 0) {
            if (!invincible) getHealth().increase(amount);
            if (amount > 0 && h != null && h == Hitmark.MISS) h = Hitmark.HIT;
    ...
    public void appendDamage(Entity entity, int damage, Hitmark h) {
        ...
        if (getAttributes().getBoolean(TobInstance.TOB_DEAD_ATTR_KEY, false)) {
            if (Boundary.isIn(this, TobConstants.ALL_BOUNDARIES) && getInstance() != null) return;
        ...
        if (h == Hitmark.HIT || h == Hitmark.MISS) Degrade.degradeDefending(this);

        if (damage > 1 && playerEquipment[Player.playerFeet] == 10558 && !getPosition().inWild()) damage *= .85;
        else if (damage > 1 && playerEquipment[Player.playerHands] == 13372 && !getPosition().inWild()) damage *= .85;
        ...
        if (getHealth().getCurrentHealth() - damage < 0) damage = getHealth().getCurrentHealth();
        ...
        if (teleTimer <= 0) {
            if (!invincible) getHealth().reduce(damage);
            if (damage > 0 && h != null && h == Hitmark.MISS) h = Hitmark.HIT;
            if (!hitUpdateRequired) {
                hitUpdateRequired = true;
                hitDiff = damage;
                hitmark1 = h;
    ...
    protected void appendHitUpdate(Stream str) {
        ...
        if (hitmark1 == null) str.writeByteA(0);
        else str.writeByteA(hitmark1.getId());
        if (getHealth().getCurrentHealth() <= 0) isDead = true;
    ...
    protected void appendHitUpdate2(Stream str) {
        ...
        if (hitmark2 == null) str.writeByteS(0);
        else str.writeByteS(hitmark2.getId());
        if (getHealth().getCurrentHealth() <= 0) isDead = true;
    ...
    public void setForceMovement(int xOffset, int yOffset, int speedOne, int speedTwo, String directionSet, int animation) {
        if (isForceMovementActive() || forceMovement) return;
        stopMovement();
        ...
        Server.getEventHandler().submit(new Event<Player>("force_movement", this, ticks) {
            ...
            public void execute() {
                ...
                } else MeleeData.setWeaponAnimations(attachment);
                forceMovement = false;
                super.stop();
            }
        });
    ...
    public void appendPlayerUpdateBlock(Stream str) {
        ...
        int updateMask = 0;
        if (forceMovement) updateMask |= 1024;
        if (isGfxUpdateRequired()) updateMask |= 256;
        if (isAnimationUpdateRequired()) updateMask |= 8;
        if (forcedChatUpdateRequired) updateMask |= 4;
        if (isChatTextUpdateRequired()) updateMask |= 128;
        if (isAppearanceUpdateRequired()) updateMask |= 16;
        if (faceUpdateRequired) updateMask |= 1;
        if (FocusPointX != -1) updateMask |= 2;
        if (hitUpdateRequired) updateMask |= 32;
        if (hitUpdateRequired2) updateMask |= 512;
        if (updateMask >= 256) {
            updateMask |= 64;
            str.writeByte(updateMask & 255);
            str.writeByte(updateMask >> 8);
        } else str.writeByte(updateMask);
        if (forceMovement) appendMask400Update(str);
        if (isGfxUpdateRequired()) appendMask100Update(str);
        if (isAnimationUpdateRequired()) appendAnimationRequest(str);
        if (forcedChatUpdateRequired) appendForcedChat(str);
        if (isChatTextUpdateRequired()) appendPlayerChatText(str);
        if (faceUpdateRequired) appendFaceUpdate(str);
        if (isAppearanceUpdateRequired()) appendPlayerAppearance(str);
    ...
    public int getDefenceBonus(CombatType combatType, Entity attacker) {
        if (combatType == CombatType.RANGE) return getItems().getBonus(Bonus.DEFENCE_RANGED);
        else if (combatType == CombatType.MAGE) return getItems().getBonus(Bonus.DEFENCE_MAGIC);
    ...
    public RightGroup getRights() {
        if (rights == null) rights = new RightGroup(this, Right.PLAYER);
        return rights;
    }
    ...
    public Titles getTitles() {
        if (titles == null) titles = new Titles(this);
        return titles;
    }
    ...
    public MiniArboBox getMiniArboBox() {
        return miniArboBox;
    }

    public MiniCoxBox getMiniCoxBox() {
        return miniCoxBox;
    }

    public MiniDonoBox getMiniDonoBox() {
        return miniDonoBox;
    }

    public MiniNormalMysteryBox getMiniNormalMysteryBox() {
        return miniNormalMysteryBox;
    }

    public MiniSmb getMiniSmb() {
        return miniSmb;
    }

    public MiniTobBox getMiniTobBox() {
        return miniTobBox;
    }

    public MiniUltraBox getMiniUltraBox() {
        return miniUltraBox;
    }
    ...
    public void replenishSkill(int skillId, int amount) {
        if (skillId < 0 || skillId > playerLevel.length - 1) return;
        int maximum = getLevelForXP(playerXP[skillId]);
        if (playerLevel[skillId] == maximum) return;
        playerLevel[skillId] += amount;
        if (playerLevel[skillId] > maximum) playerLevel[skillId] = maximum;
    ...
    public void clearUpPlayerNPCsForLogout() {
        for (NPC n : NPCHandler.npcs)
            if (n != null) if (n.spawnedBy == this.getIndex()) {
                if (n.isPet)
                    continue;
                if (n.isThrall)
                    continue;
    ...
    public void removeFromInstance() {
        if (getInstance() != null) getInstance().remove(this);
    }
    ...
    public void setRunEnergy(int runEnergy, boolean update) {
        ...
        if (update) getPA().updateRunEnergy();
    }
    ...
    public Inferno getInferno() {
        if (getInstance() != null && getInstance() instanceof Inferno) return (Inferno) getInstance();
        return null;
    }
    ...
    public io.xeros.content.dwarfleaguecannon.Cannon getLeagueCannon() {
        return dwarfCannon;
    }

    public void setLeagueCannon(io.xeros.content.dwarfleaguecannon.Cannon dwarfCannon) {
    ...
    public String getDisplayNameFormatted() {
        if (hideDonor) return getDisplayName();
    ...
    public int donatorVaultLootCount = 0;
