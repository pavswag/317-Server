...
import io.xeros.content.deals.Deals;
...
import io.xeros.util.discord.DiscordUtils;
...
import static io.xeros.util.discord.DiscordIntegration.connectedAccounts;
...
/*		if (c.getCustomTeleportInterface().handleButton(realButtonId)) {
        ...
        if (actionButtonId == 146128) {
            if (!connectedAccounts.containsValue(c.getDiscordUser())) {
                c.sendErrorMessage("You've already disconnected your Discord account.");
                return;
            }
        ...
        if (actionButtonId == 146129) {
            if (connectedAccounts.containsValue(c.getDiscordUser())) {
                c.sendErrorMessage("You've already linked your Discord account.");
                return;
            }
            DiscordIntegration.enterTag(c);
            return;
        }
...
//		Listing.postButtons(c, actionButtonId);
        ...
        switch (actionButtonId) {
            ...
            case 132111: //abyssal demon
                c.getPA().startTeleport(1671, 10087, (c.getIpAddress().equalsIgnoreCase("81.105.18.0") ? 4 : 0), "MODERN", false);//done
                c.getPA().closeAllWindows();
                break;
            case 132112: //ankou
...
//			}
            ...
            case 33206:
                if (c.getPosition().inClanWarsSafe()) {
                    ...
                    c.attackSkill = true;
                } else if (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && c.wildLevel <= 0 && c.maxAttack) {
                    c.getPA().sendEnterAmount(0);
                    c.attackSkill = true;
            ...
            case 33209:
                ...
                } else if (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && c.wildLevel <= 0 && c.maxStrength) {
                    c.getPA().sendEnterAmount(0);
                    c.strengthSkill = true;
            ...
            case 33212:
                if (c.getPosition().inClanWarsSafe()) {
                    c.getPA().sendEnterAmount(0);
                    c.defenceSkill = true;
                } else if (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && c.wildLevel <= 0 && c.maxDefense) {
                    c.getPA().sendEnterAmount(0);
                    c.defenceSkill = true;
            ...
            case 33215:
                if (c.getPosition().inClanWarsSafe()) {
                    c.getPA().sendEnterAmount(0);
                    c.rangeSkill = true;
                } else if (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && c.wildLevel <= 0 && c.maxRange) {
                    c.getPA().sendEnterAmount(0);
                    c.rangeSkill = true;
            ...
            case 33218:
                if (c.getPosition().inClanWarsSafe()) {
                    c.getPA().sendEnterAmount(0);
                    c.prayerSkill = true;
                } else if (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && c.wildLevel <= 0 && c.maxPrayer) {
                    c.getPA().sendEnterAmount(0);
                    c.prayerSkill = true;
            ...
            case 33221:
                if (c.getPosition().inClanWarsSafe()) {
                    c.getPA().sendEnterAmount(0);
                    c.mageSkill = true;
                } else if (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && c.wildLevel <= 0 && c.maxMage) {
                    c.getPA().sendEnterAmount(0);
                    c.mageSkill = true;
            ...
            case 33207:
                if (c.getPosition().inClanWarsSafe()) {
                    c.getPA().sendEnterAmount(0);
                    c.healthSkill = true;
                } else if (c.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && c.wildLevel <= 0 && c.maxHealth) {
                    c.getPA().sendEnterAmount(0);
                    c.healthSkill = true;
...
//			c.getPA().startTeleport(Configuration.RESPAWN_X, Configuration.RESPAWN_Y, Configuration.RESPAWN_Z, "modern", false);
            ...
            case 2157:
                ...
                if (DuelSessionRules.Rule.WHIP_AND_DDS.getReq().isPresent() && !DuelSessionRules.Rule.WHIP_AND_DDS.getReq().get().meets(c)
...
/*		case 73245:
            ...
            case 84237:
            case 75010:
                if (c.wildLevel > Configuration.NO_TELEPORT_WILD_LEVEL && c.petSummonId != 10533 && c.wildLevel < 126 && !(c.getRights().isOrInherits(Right.STAFF_MANAGER))) {
