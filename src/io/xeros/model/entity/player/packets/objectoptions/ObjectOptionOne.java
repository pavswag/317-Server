...
import io.xeros.content.afkzone.Afk;
...
import io.xeros.content.bosses.godwars.impl.ArmadylInstance;
import io.xeros.content.bosses.godwars.impl.BandosInstance;
import io.xeros.content.bosses.godwars.impl.SaradominInstance;
import io.xeros.content.bosses.godwars.impl.ZamorakInstance;
...
import io.xeros.model.world.areas.impl.OnyxZone;
...
//		System.out.println(objectType + " / " + c.getDisplayName());
        ...
        if (c.wildLevel > 0 && c.underAttackByPlayer > 0 && objectType != 26763 && objectType != 1727 && objectType != 18987/* && objectType != 1816 && objectType != 5959*/) {//
            return;
        }

        if (c.wildLevel > 0 && c.underAttackByNpc > 0 && objectType != 26763 && objectType != 1727 && objectType != 18987 && objectType != 1816 && objectType != 5959) {
            return;
        }
...
/*			if (days < 1) {
            ...
            lines.add("Welcome to Gambling on Turmoil.");
        ...
        if (object.getObjectId() == 4157) {
            ...
            if (c.getX() >= 2852) {
                ...
                c.moveTo(new Position(2846, c.getY(), c.getHeight()));
            }
        }
        ...
        if (object.getObjectId() == 120) {//balloon
          //  Server.getGlobalObjects().remove(new GlobalObject(120, 1241, 1242, 0, 0, 10, -1, -1).setInstance(this));
        ...
        if (new OnyxZone().handledClickObject(c, def)) return;
        ...
        switch (objectType) {
            case 4470:
                if (c.getY() == 2721) {
                    if ((c.getX() >= 2064 && c.getX() <= 2067) ||
                            (c.getX() >= 2092 && c.getX() <= 2095) ||
                            (c.getX() >= 2122 && c.getX() <= 2125) ||
                            (c.getX() >= 2142 && c.getX() <= 2145) ||
                            (c.getX() >= 2162 && c.getX() <= 2165))
                        c.getPA().movePlayer(c.getX(), c.getY() + 2, c.getHeight());
                }
                if (c.getY() == 2723) {
                    if ((c.getX() >= 2064 && c.getX() <= 2067) ||
                            (c.getX() >= 2092 && c.getX() <= 2095) ||
            ...
            case 49138:
                if (c.getInstance() != null) {
                    c.moveTo(new Position(3039, 6434, 0));
                } else {
            ...
            case 48234:
                if (Boundary.isIn(object, DonorVault.area) && Boundary.isIn(c, DonorVault.area)) {
                    if (c.DonorVaultObjects.size() >= 20) {
                        ...
                        c.sendMessage("@red@You have completed your personal vault! Thank you for supporting Turmoil!");
                    ...
                    if (c.donatorVaultLootCount >= 20) {

                        final String message =
                                "Offered Name:  **__" + c.getLoginName().toUpperCase() + "__** \n" +
                                        "Loot Count: **__" + c.donatorVaultLootCount + "__** \n" +
                                        "TimeStamp: " + "**__<t:" + TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis()) + ":f>__**";

                        Discord.sendEmbeddedServerLog(
                                Discord.DONATOR_VAULT_CHANNEL,
                                Color.RED,
                                "Donator Vault Logs",
                                message,
                                c.getLoginName(),
                                "IP Address: " + c.getIpAddress());

                        c.donatorVaultLootCount = 0;
                        c.getPA().startTeleport(new Position(3089, 3503), "modern", true);
                    }
            ...
            case 4874:
            case 11730:
                c.getThieving().steal(Stall.Cake, location);
                c.objectDistance = 1;
                break;
            case 8929:
...
/*				if (!(System.currentTimeMillis() - c.chestDelay > 2000)) {
            ...
            case 47593:
                if (!c.getItems().playerHasItem(995, 10000000)) {
                    c.sendMessage("You need 10M to enter.");
                    return;
                }
                c.getPA().movePlayer(2072, 6368, 0);
                c.getItems().deleteItem2(995, 10000000);
                c.sendMessage("You paid the fee to slay the beast.");
                break;
            case 3840:
...
/*			case 1521:
			...
			case 14749:
			case 14750:
				io.xeros.objects.DoorHandler.handleDoor(c, object);
				break;*/
            case 14735:
            ...
            case 544:
                if (c.getX() == 2688) {
                    c.facePosition(2689, 3697);
                    AgilityHandler.delayEmote(c, "JUMP", 2691, 3697, 0, 1);
                    c.startAnimation(3067);
                } else {
                    c.facePosition(2689, 3697);
                    AgilityHandler.delayEmote(c, "JUMP", 2688, 3697, 0, 1);
                    c.startAnimation(3067);
                }
                break;
            ...
            case 52394:
                ...
                c.sendMessage("You feel rejuvenated.");
                break;
...
/*				if (true) {
            ...
            case 43749:
                ...
                if (c.amDonated >= 100) {
                    ...
                    c.sendMessage("You need to have $100 Donor or higher to access this area.");
                break;
            case 5090:
            case 20884:
                if (c.playerLevel[16] < 30) {
