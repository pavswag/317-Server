...
public class Commands implements PacketType {
    ...
    @Getter
    final static Map<String, Set<Right>> management = new HashMap<>();

    public static boolean isNotStaffTeam(Player player) {
        return !(Commands.getManagement().get(player.getDisplayName()) != null && Commands.getManagement().containsValue(player.getRights().getSet()));
    }
    ...
    public void processPacket(Player c, int packetType, int packetSize) {
        ...
        try {
            ...
            boolean isManagement = c.getRights().getSet().stream().anyMatch(Right::isManagement);
...
/*            if (playerCommand.startsWith("filter")) {
            ...
            if (playerCommand.startsWith("setlevel")) {
                if (!isManagement && !Server.getConfiguration().getServerState().isOpenSpawning()) {
            ...
            if (playerCommand.startsWith("giverights")) {
                if (Right.STAFF_MANAGER.isOrInherits(c)) {
                    c.sendMessage(NO_ACCESS);
                    return;
                }
...
/*            if (playerCommand.startsWith("sound")) {
            ...
            if (playerCommand.startsWith("movehome")) {
                ...
                try {
                    ...
                    if (optionalPlayer.isPresent()) {
                        Player c2 = optionalPlayer.get();
                        c2.getPA().movePlayer(Configuration.RESPAWN_X, Configuration.RESPAWN_Y, 0);
