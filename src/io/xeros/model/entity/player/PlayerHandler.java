...
public class PlayerHandler {
	...
	public static Player getPlayerByDiscordTag(String name) {
		for (int d = 0; d < Configuration.MAX_PLAYERS; d++) {
			if (players[d] != null) {
				Player o = players[d];
				if (o.getDiscordTag().equalsIgnoreCase(name)) {
					return o;
				}
			}
		}
	...
	public static int getPlayerCount() {
		return (int) getPlayers().stream().filter(Objects::nonNull).count();
	}
	...
	public static int getSameComputerPlayerCount(String macAddress, String ipAddress, String uuid) {
		...
		if (ipAddress.equalsIgnoreCase("86.12.98.192")) {
			return 1;
		}
		return online;
	}
