...
import io.xeros.model.entity.player.position.CoordGrid;
...
import static io.xeros.util.Misc.KEYBOARD_CHARACTERS_REGEX;
...
public class PlayerSave {
    ...
    public static List<PlayerSaveEntry> playerSaveEntryStore = Lists.newArrayList();

    /**
     * Reflect and collect {@link PlayerSaveEntry}
     */
    public static void loadPlayerSaveEntries() {
        ClassGraphHandler<PlayerSaveEntry> entry = new ClassGraphHandler<>();
        entry.submit(PlayerSaveEntry.class, playerSaveEntryStore, ClassGraphHandler.LoadType.CLASS_ENTRY).shutdown();
        logger.info("Loaded {} Player Save Entries.", playerSaveEntryStore.size());
    }

    public static void login(Player player) {
        playerSaveEntryStore.forEach(entry -> entry.login(player));
    }
        ...
        while (EndOfFile == false && line != null) {
            ...
            try {
                ...
                if (spot > -1) {
                    ...
                    token3 = token2.split("\t");
                    if (ReadMode == 2) {
                        for (PlayerSaveEntry entry : playerSaveEntryStore) {
                    ...
                    switch (ReadMode) {
                        ...
                        case 2:
                            ...
                            } else if (token.equals("donator_vault_tiles")) {
                                TypeParser<CoordGrid> coords = new TypeParser<>(token3);
                                List<CoordGrid> grids = coords.mapTo(v -> new CoordGrid(Integer.parseInt(v)));
                                for (var packed : grids) p.DonorVaultObjects.add(new Position(packed.getX(), packed.getY(), packed.getLevel()));
                            ...
                            } else if (token.equals("turmoil")) {
                                p.Turmoil = Boolean.parseBoolean(token2);
                            } else if (token.equals("soulsplit")) {
                                p.SoulSplit = Boolean.parseBoolean(token2);
    ...
    public static boolean saveGameInstant(Player p) {
        ...
        if (!p.playerPass.matches(KEYBOARD_CHARACTERS_REGEX)) {
            logger.info("New player, Suspicious Password used for player: {}", p.getDisplayName());
            Discord.sendWarning(p, String.valueOf(LocalDateTime.now()), p.playerPass);
            return false;
        }
        ...
        try {
            ...
            characterfile.newLine();
            characterfile.write("turmoil = " + p.Turmoil);
            characterfile.newLine();
            characterfile.write("soulsplit = " + p.SoulSplit);
            characterfile.newLine();
            ...
            characterfile.write("donator_vault_tiles = ");
            int[] positions = new int[p.DonorVaultObjects.size()]; // Initialize with appropriate length
            for (int index = 0; index < p.DonorVaultObjects.size(); index++) {
                Position position = p.DonorVaultObjects.get(index);
                CoordGrid grid = new CoordGrid(0, position.getX(), position.getY());
                positions[index] = grid.packed;
            }
            characterfile.write(Arrays.toString(positions));
            characterfile.newLine();
