...
import io.xeros.content.skills.agility.PyramidPlunder;
	...
	public void sendSound(int soundId, SoundType soundType, Entity source) {
		if (player.getOutStream() != null) {
			if (player.debugMessage) {
				player.sendMessage("Sent sound " + soundId);
			}
			player.getOutStream().createFrame(12);
			player.getOutStream().writeUnsignedWord(soundId);
			player.getOutStream().writeByte(soundType.ordinal());
			int index = 0;
			if (source != null) {
				index = source.getIndex() + (source.isPlayer() ? Short.MAX_VALUE : 0);
			}
			player.getOutStream().writeUnsignedWord(index);
		}
	}
...
//			c.getOutStream().writeUnsignedWord(volume);
	...
	public void sendSong(int id) {
		if (player.getOutStream() != null && id != -1) {
			if (player.debugMessage) {
				player.sendMessage("Sent sound " + id);
			}
	...
	public void destroyInterface(String config) {
		int itemId = player.destroyItem.getItemId();
...
//
	...
	public void itemOnInterface(int interfaceChild, int zoom, int itemId) {
		if (player.getOutStream() != null) {
			...
			player.getOutStream().writeUnsignedWord(zoom);
			player.getOutStream().writeUnsignedWord(itemId);
			player.flushOutStream();
		}
	}
	...
	public void itemOnInterface(int item, int amount, int frame, int slot) {
		if (player.getOutStream() != null) {
			...
			player.outStream.writeUnsignedWord(frame);
			player.outStream.writeDWord(slot);
			player.outStream.writeUnsignedWord(item + 1);
...
//			System.out.println("ItemOnInterface");
	...
	public void sendTabAreaOverlayInterface(int interfaceId) {
		if (player.getOutStream() != null) {
			...
			player.outStream.writeUnsignedWord(interfaceId);
			player.flushOutStream();
		}
	}
...
/*	public void initWheelOfFortune(int wheelInterfaceId, int winningIndex, int[] items) {
	...
	public void setScrollableMaxHeight(int interfaceId, int scrollMax) {
		if (player.getOutStream() != null) {
			...
			player.getOutStream().writeUnsignedWord(interfaceId);
			player.getOutStream().writeUnsignedWord(scrollMax);
			player.flushOutStream();
		}
	}
	...
	public void sendFrame248(int MainFrame, int SubFrame) {
		...
		if (player.getOutStream() != null && player != null) {
			...
			player.getOutStream().writeUnsignedWord(SubFrame);
			player.flushOutStream();
	...
	public void sendFrame246(int MainFrame, int SubFrame, int SubFrame2) {
		...
		if (player.getOutStream() != null && player != null) {
			...
			player.getOutStream().writeUnsignedWord(SubFrame);
			player.getOutStream().writeUnsignedWord(SubFrame2);
			player.flushOutStream();
		}
	}
	...
	public void initWheelOfFortune(int interfaceId, int index, int[] items){
		if (player.getOutStream() != null) {
			player.getOutStream().createFrame(225);
			player.getOutStream().writeUnsignedWord(interfaceId);
			player.getOutStream().writeByte(index);
			player.getOutStream().writeByte(items.length);
			IntStream.of(items).forEach(i -> player.getOutStream().writeUnsignedWord(i));
			player.flushOutStream();
		}
	}
	...
	public void sendInterfaceAnimation(int interfaceId, int animation) {
		...
		if (player.getOutStream() != null && player != null) {
			...
			player.getOutStream().writeUnsignedWord(interfaceId);
			player.getOutStream().writeUnsignedWord(animation);
			player.flushOutStream();
		}
	}
	...
	public void sendFrame70(int i, int o, int id) {
		...
		if (player.getOutStream() != null && player != null) {
			...
			player.getOutStream().writeUnsignedWord(i);
	...
	public void createPlayerHints(int type, int id) {
		if (player.getOutStream() != null && player != null) {
			...
			player.getOutStream().writeUnsignedWord(id);
	...
	public void createObjectHints(int x, int y, int height, int pos) {
		...
		if (player.getOutStream() != null && player != null) {
			...
			player.getOutStream().writeUnsignedWord(x);
			player.getOutStream().writeUnsignedWord(y);
	...
	public void sendFrame34(int id, int slot, int column, int amount) {
		...
		player.getOutStream().writeUnsignedWord(column);
		player.getOutStream().writeDWord(slot);
		player.getOutStream().writeUnsignedWord(id + 1);
		player.getOutStream().writeByte(255);
...
//		System.out.println("id = " + id + ", slot = " + slot + ", column = " + column + ", amount = " + amount );
	...
	public void createProjectile(int x, int y, int offX, int offY, int angle, int speed, int gfxMoving,
		...
		if (player.getOutStream() != null && player != null) {
			...
			player.getOutStream().writeUnsignedWord(lockon);
			player.getOutStream().writeUnsignedWord(gfxMoving);
			player.getOutStream().writeByte(startHeight);
			player.getOutStream().writeByte(endHeight);
			player.getOutStream().writeUnsignedWord(time);
			player.getOutStream().writeUnsignedWord(speed);
	...
	public void sendProjectile(Projectile projectile) {
			...
			player.getOutStream().writeUnsignedWord(projectile.getLockon());
			player.getOutStream().writeUnsignedWord(projectile.getProjectileId());
			player.getOutStream().writeByte(projectile.getStartHeight());
			player.getOutStream().writeByte(projectile.getEndHeight());
			player.getOutStream().writeUnsignedWord(projectile.getDelay());
			player.getOutStream().writeUnsignedWord(projectile.getSpeed());
	...
	public void sendProjectile(int startX, int startY, int destX, int destY, int angle, int duration, int gfxMoving, int startHeight, int endHeight, int lockon, int startDelay, int slope, int creatorSize, int startDistanceOffset) {
		if (player.getOutStream() != null) {
			player.getOutStream().createFrame(85);
			player.getOutStream().writeByteC((startY - (player.getMapRegionY() * 8)));
			player.getOutStream().writeByteC((startX - (player.getMapRegionX() * 8)));
			player.getOutStream().createFrame(117);
			player.getOutStream().writeByte(angle);
			player.getOutStream().writeByte(destY);
			player.getOutStream().writeByte(destX);
			player.getOutStream().writeUnsignedWord(lockon);
			player.getOutStream().writeUnsignedWord(gfxMoving);
			player.getOutStream().writeByte(startHeight);
			player.getOutStream().writeByte(endHeight);
			player.getOutStream().writeUnsignedWord(startDelay);
			player.getOutStream().writeUnsignedWord(duration);
			player.getOutStream().writeByte(slope);
			player.getOutStream().writeByte(startDistanceOffset);
			player.flushOutStream();
		}
	}
	...
	public void mysteryBoxItemOnInterface(int item, int amount , int frame, int slot) {
		...
		player.getOutStream().writeDWord(slot);
		player.getOutStream().writeUnsignedWord(item + 1);
		player.getOutStream().writeByte(255);
		player.getOutStream().writeDWord(amount);
		player.getOutStream().endFrameVarSizeWord();
	}
	...
	public void createPlayersProjectile(int x, int y, int offX, int offY, int angle, int speed, int gfxMoving,
		...
		PlayerHandler
				...
				.filter(p -> player.sameInstance(p))
				.forEach(p -> {
	 ...
	 ** GFX
	 **/
	public void stillGfx(int id, Position position, int height, int time) {
		stillGfx(id, position.getX(), position.getY(), height, time);
	}
	...
	public void sendItemToSlotWithOpacity(int frame, int item, int slot, int amount, boolean opaque) {
		...
		player.outStream.writeUnsignedWord(bitpackedValue);
		player.outStream.writeByte(255);
		player.outStream.writeDWord(amount);
		player.outStream.endFrameVarSizeWord();
	}
	...
	public void sendStringContainer(int containerInterfaceId, String...strings) {
		if (player.outStream != null) {
			...
			player.outStream.writeUnsignedWord(containerInterfaceId);
			player.outStream.writeUnsignedWord(strings.length);
	...
	public void sendInterfaceSet(int mainScreenInterface, int inventoryInterface) {
		...
		player.getOutStream().writeUnsignedWord(inventoryInterface);
	...
	public void castVengeance() {
		...
		refreshSkill(6);
		player.getPA().sendSound(2907, SoundType.SOUND);
		player.getPA().sendConfig(2450, 1);//but its sending this
		player.getPA().sendConfig(2451, 1);
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			@Override
			public void execute(CycleEventContainer container) {
				player.getPA().sendConfig(2451, 0);//this is happening after timer ends
				container.stop();
			}
		}, 50);
		player.vengOn = true;
		...
		switch (alch) {
			case "low":
				if (System.currentTimeMillis() - player.alchDelay > 1000) {
					...
					refreshSkill(6);
					player.getPA().sendSound(98, SoundType.SOUND);
				}
				break;

			case "high":
				if (System.currentTimeMillis() - player.alchDelay > 500) {
					for (int[] items : EquipmentSet.IRON_MAN_ARMOUR.getEquipment()) {
						if (Misc.linearSearch(items, itemId) > -1) {
							player.sendMessage("You cannot alch iron man amour.");
							break;
					...
					player.alchDelay = System.currentTimeMillis();
					sendFrame106(6);
					player.getPA().sendSound(97, SoundType.SOUND);
		...
		for (int[] aBoltData : boltData) {
			if (spell == aBoltData[0]) {
				if (bolt == aBoltData[1]) {
					...
					player.getPA().addSkillXPMultiplied(aBoltData[2] * amount, 6, true);
...
/*			if (Boundary.isIn(c, Boundary.ICE_PATH) || Boundary.isIn(c, Boundary.ICE_PATH_TOP) && c.heightLevel > 0) {
			...
			if (!player.isDead && player.teleTimer == 0 && player.respawnTimer == -6) {
				...
				player.getPA().sendSound(2400, SoundType.SOUND);
			}
...
/*		if (Boundary.isIn(c, Boundary.OUTLAST_HUT)) {
	...
	public boolean canTeleport(String teleportType) {
		...
		if (Boundary.TOURNY_LOBBY.in(player) && !Boundary.TOURNAMENT_LOBBIES_AND_AREAS.in(player) ) {
			player.sendMessage("A magical force blocks your teleport, you'll have to log out!!!");
			return false;
		}
		if (WGManager.getSingleton().isInArenaBounds(player) || WGManager.getSingleton().isInLobbyBounds(player)) {
			player.sendMessage("You cannot teleport when in the WeaponGames arena.");
			return false;
		}
		...
		if (player.getPosition().inWild() && !(player.getRights().isOrInherits(Right.STAFF_MANAGER))) {
			if (!teleportType.equals("glory") && !teleportType.equals("obelisk") && !teleportType.equals("pod") && player.petSummonId != 10533) {
			...
			} else if (!teleportType.equals("obelisk") && player.petSummonId != 10533 && !(player.getRights().isOrInherits(Right.STAFF_MANAGER))) {
	...
	private void startTeleportFinal(int x, int y, int height, String teleportType, boolean homeTeleport) {
		...
		if (player.getPosition().inWild() && player.isTeleblocked() && !(player.getRights().isOrInherits(Right.STAFF_MANAGER))) {
			return;
		}
		if (player.isFping()) {
		...
		if (player.getItems().getWeapon() == 22816 || player.getItems().getWeapon() == 22817) {
			player.getItems().equipItem(-1, 0, Player.playerWeapon);
		} else if (player.getItems().getInventoryCount(22816) > 0 || player.getItems().getInventoryCount(22817) > 0) {
			player.getItems().deleteItem2(22816, 1);
			player.getItems().deleteItem2(22817, 1);
		}
		if (player.inPyramidPlunder) {
			PyramidPlunder.endMinigame(player);
			return;
		}
		...
		if (player.getXeric() != null) {
			player.getXeric().leaveGame(player, false);
		}
	...
	public void followNpc() {
		...
		if (player.morphed) return;
		if (player.freezeTimer > 0) return;
		if (player.isDead || player.playerLevel[3] <= 0) return;
						...
						for (int i = 0; i < loop - (k == 0 ? 1 : 0); i++) {
							...
							if (Misc.distance(x3, y3, x, y) < lowDist) {
								boolean allowsInteraction = !projectile
										|| projectile &&  PathChecker.raycast(player, npc, true);
	...
	public void levelUp(int skill) {
		...
		int totalLevel = (getLevelForXP(player.playerXP[0]) + getLevelForXP(player.playerXP[1]) + getLevelForXP(player.playerXP[2])
				...
				+ getLevelForXP(player.playerXP[21])+ getLevelForXP(player.playerXP[22])+ getLevelForXP(player.playerXP[23]));
		...
		if (player.getRights().isNotAdmin()) {
			...
			if (player.maxRequirements(player) && skill < 22) {
	...
	public boolean hasSkillLevels(SkillLevel...skillLevels) {
		return Arrays.stream(skillLevels).allMatch(skill -> getLevelForXP(player.playerXP[skill.getSkill().getId()]) >= skill.getLevel());
	}
	...
	public boolean addSkillXP(int amount, int skill, boolean dropExperience) {
		...
		if (skill == Skill.CRAFTING.getId()) {
			if (Fletching.getFletchEquipmentCount(player) > 0) {
				amount *= Fletching.getFletchEquipmentCount(player);
			}
		}

		if (Boundary.isIn(player, Boundary.DONATOR_ZONE_NEW) || Boundary.isIn(player, Boundary.DONATOR_ZONE)) {
		...
		if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33080) && skill == Skill.FLETCHING.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33087) && skill == Skill.THIEVING.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33088) && skill == Skill.CRAFTING.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33089) && skill == Skill.COOKING.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33093) && skill == Skill.DEMON_HUNTER.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33094) && skill == Skill.SLAYER.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33095) && skill == Skill.FIREMAKING.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33096) && skill == Skill.HUNTER.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33097) && skill == Skill.MINING.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33098) && skill == Skill.WOODCUTTING.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33099) && skill == Skill.FISHING.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		} else if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33101) && skill == Skill.PRAYER.getId() && Misc.random(0, 20) <= 3) {
			amount *= 2.2;
		}
		if (player.getPerkSytem().gameItems.stream().anyMatch(item -> item.getId() == 33122)) {
			amount *= 2;
		}

		if (player.fullCosmetic()) {
			amount *= 1.5;
		}

		if (player.fullSweet() && Christmas.isChristmas()) {
			amount *= 1.5;
		}

		if (PrestigePerks.hasRelic(player, PrestigePerks.EXPERIENCE_BONUS1)) {
			amount *= 1.15;
		}

		if (PrestigePerks.hasRelic(player, PrestigePerks.EXPERIENCE_BONUS2)) {
			amount *= 1.15;
		}

		if (PrestigePerks.hasRelic(player, PrestigePerks.EXPERIENCE_BONUS3)) {
			amount *= 1.15;
		}

		if (PrestigePerks.hasRelic(player, PrestigePerks.EXPERIENCE_DAMAGE_BONUS1)) {
			amount *= 1.5;
		}
		...
		if (player.EliteCentBoost > 0) {
			amount *= 2;
		}

		if (player.daily2xXPGain > 0) {
			amount *= 2;
		}
...
/*		if (c.getMode().getType().isStandardRate(Skill.forId(skill))) {
		...
		if (oldLevel < getLevelForXP(player.playerXP[skill])) {
			...
			CharacterInformationPanel.Update(player);
	...
	public void drawHeadicon(int type, int index) {
		...
		if (type == 1 || type == 10) {
			player.outStream.writeUnsignedWord(index);
			player.outStream.writeUnsignedWord(k);
			player.outStream.writeByte(l);
		} else {
			player.outStream.writeUnsignedWord(k);
			player.outStream.writeUnsignedWord(l);
	...
	public void displayPlayerHintIcon(int index) {
		...
		player.outStream.writeUnsignedWord(index);
		player.outStream.writeUnsignedWord(0);
	...
	public void displayStaticIcon(int type, int x, int y, int iconHeight) {
		...
		player.outStream.createFrame(254);
		player.outStream.writeByte(type);
		player.outStream.writeUnsignedWord(x);
		player.outStream.writeUnsignedWord(y);
		...
		if (player.getOutStream() != null) {
			...
			for (Object param : params) {
				if (param instanceof Integer) {
					player.getOutStream().writeDWord((int) param);//nvm thats int right? dword = int
				}else if(param instanceof Boolean) {
					player.getOutStream().writeDWord(Boolean.TRUE.equals(param) ? 1 : 0);
				} else {
					player.getOutStream().writeNullTerminatedString(param.toString());
				}
			}

			player.getOutStream().writeUnsignedWord(scriptId);
			player.getOutStream().endFrameVarSizeWord();
			player.flushOutStream();
		}
	}
	...
	public void useOperate(int itemId) {
		...
		switch (itemId) {
			...
			case 13136:
				...
				if (player.wildLevel > Configuration.NO_TELEPORT_WILD_LEVEL && player.petSummonId != 10533 && !(player.getRights().isOrInherits(Right.STAFF_MANAGER))) {
					player.sendMessage(
			...
			case 11111:
				if (player.getMode().equals(Mode.forType(ModeType.HARDCORE_WILDYMAN)) || player.getMode().equals(Mode.forType(ModeType.WILDYMAN))) {
					player.sendMessage("You cannot use a necklace of skills.");
					return;
				}
				if (player.getPosition().inClanWars() || player.getPosition().inClanWarsSafe()) {
					player.sendMessage("@cr10@You can not teleport from here, speak to the doomsayer to leave.");
	...
	public void stillCamera(int x, int y, int height, int speed, int angle) {
		...
		player.outStream.writeUnsignedWord(height);
...
/*		if (c == null || c.isDisconnected()) {
	...
	public void sendSlayerCount(int id) {
		...
		if (player.getSlayer().getTaskAmount() > 0 && player.getSlayer().getTask().isPresent()) {
			stream.createFrame(211);
			stream.writeWordBigEndian(id);
			stream.writeDWord(player.getSlayer().getTaskAmount());
			player.flushOutStream();
		}
	}
