...
public class NpcAggression {
    ...
    public static void doAggression(NPC npc, NPCHandler npcHandler) {
        if ((isAggressive(npc) || npc.getBehaviour().isAggressive())
            ...
            for (Player player : PlayerHandler.players) {
                ...
                if (player.heightLevel != npc.heightLevel) continue;
                if (Boundary.isIn(player, Boundary.DIAMOND_ZONE) || Boundary.isIn(npc, Boundary.DIAMOND_ZONE)) return;
                if (Boundary.isIn(player, Boundary.ONYX_ZONE_NEW) || Boundary.isIn(npc, Boundary.ONYX_ZONE_NEW)) return;
                if (Boundary.isIn(player, Boundary.ZENYTE_ZONE) || Boundary.isIn(npc, Boundary.ZENYTE_ZONE)) return;
                if (Boundary.isIn(player, Boundary.DONATOR_ZONE) || Boundary.isIn(npc, Boundary.DONATOR_ZONE)) return;

                if (npc.getNpcId() == FragmentOfSeren.NPC_ID) return;
                if (Boundary.isIn(player, Boundary.RESOURCE_AREA_BOUNDARY)) continue;
                // Optimization
                if (player.distanceToPoint(npc.absX, npc.absY) > 26) continue;
                ...
                if (!Boundary.isIn(player, Boundary.GODWARS_BOSSROOMS))
                    if (god != null && player.getPosition().inGodwars() && player.getEquippedGodItems() != null
                            && player.getEquippedGodItems().contains(god)) continue;
                ...
                if ((player.logoutDelay > System.currentTimeMillis())) continue;

                int distanceRequired = getAggressionDistance(npc, player, npc.getSize() + 1);

                if (Boundary.isIn(npc, Boundary.CATACOMBS) && npc.getHeight() >= 4) distanceRequired = 20;
    ...
    public static int getCloseRandomPlayer(NPC npc) {
        ...
        for (int j = 0; j < PlayerHandler.players.length; j++)
            if (PlayerHandler.players[j] != null) {
                // Great Olm
                if (npc.getNpcId() == 7554 && !PlayerHandler.players[j].getPosition().inOlmRoom()) continue;
                 ...
                 */
                if (PlayerHandler.players[j].isInvisible()) continue;
                ...
                if (Boundary.isIn(npc, Boundary.CATACOMBS) && npc.getHeight() >= 4) distanceRequired = 40;
    ...
    private static int getAggressionDistance(NPC npc, Player player, int baseDistance) {
        ...
        if (Boundary.isIn(npc, Boundary.SARACHNIS_LAIR) || Boundary.isIn(npc, Boundary.MIMIC_LAIR) || Boundary.isIn(npc, Boundary.GROTESQUE_LAIR))
            return 25;
        if (Boundary.isIn(npc, Boundary.CATACOMBS)) return 5;
        if (Boundary.isIn(npc, Boundary.MOS_LEHARMLESS_CAVE1)) return 5;
        if (Boundary.isIn(npc, Boundary.MOS_LEHARMLESS_CAVE2)) return 5;
        if (Boundary.isIn(npc, Boundary.MOS_LEHARMLESS_CAVE3)) return 5;
        if (Boundary.isIn(npc, Boundary.XERIC)) return 50;
        if (Boundary.isIn(npc, ArbograveConstants.ALL_BOUNDARIES)) return 50;
        if (npc.getNpcId() > 0)
            switch (npc.getNpcId()) {
                case 7594:
                case 5630:
    ...
    public static boolean isAggressive(NPC npc) {
        ...
        if (npc.getNpcId() >= 5886 && npc.getNpcId() <= 5891) return true;
        ...
        if (Arrays.stream(DagannothMother.DAGANNOTH_MOTHER_TRANSFORMS).anyMatch(dagId -> dagId == npcId)) return true;
