...
public class NPCHandler {
    ...
    public static boolean isFightCaveNpc(NPC npc) {
        ...
        return switch (npc.getNpcId()) {
            case Wave.TZ_KEK_SPAWN, Wave.TZ_KIH, Wave.TZ_KEK, Wave.TOK_XIL, Wave.YT_MEJKOT, Wave.KET_ZEK,
                 Wave.TZTOK_JAD -> true;
            default -> false;
        };
    }
    ...
    public static boolean isInfernoNpc(NPC npc) {
        if (npc == null) return false;
        return switch (npc.getNpcId()) {
            case InfernoWaveData.JAL_NIB, InfernoWaveData.JAL_MEJRAH, InfernoWaveData.JAL_AK,
                 InfernoWaveData.JAL_AKREK_MEJ, InfernoWaveData.JAL_AKREK_XIL, InfernoWaveData.JAL_AKREK_KET,
                 InfernoWaveData.JAL_IMKOT, InfernoWaveData.JAL_XIL, InfernoWaveData.JAL_ZEK,
                 InfernoWaveData.JALTOK_JAD, InfernoWaveData.YT_HURKOT, InfernoWaveData.TZKAL_ZUK,
    ...
    public static boolean isSkotizoNpc(NPC npc) {
        if (npc == null) return false;
        return switch (npc.getNpcId()) {
            case Skotizo.SKOTIZO_ID, Skotizo.AWAKENED_ALTAR_NORTH, Skotizo.AWAKENED_ALTAR_SOUTH,
                 Skotizo.AWAKENED_ALTAR_WEST, Skotizo.AWAKENED_ALTAR_EAST, Skotizo.REANIMATED_DEMON,
                 Skotizo.DARK_ANKOU -> true;
            default -> false;
        };
    }
    ...
    public int getNpcDelay(NPC npc) {
        return switch (npc.getNpcId()) {
            case Npcs.NEX -> 4;
            case 1257 -> 4;
            case InfernoWaveData.JAL_NIB -> 4;
            case InfernoWaveData.JAL_MEJRAH -> 7;
            case InfernoWaveData.JAL_AK -> 4;
            case InfernoWaveData.JAL_AKREK_KET -> 4;
            case InfernoWaveData.JAL_AKREK_MEJ -> 4;
            case InfernoWaveData.JAL_AKREK_XIL -> 4;
            case InfernoWaveData.JAL_IMKOT -> 6;
            case InfernoWaveData.JAL_XIL -> 6;
            case InfernoWaveData.JAL_ZEK -> 6;
            case InfernoWaveData.YT_HURKOT -> 6;
            case InfernoWaveData.JALTOK_JAD -> 9;
            case InfernoWaveData.JAL_MEJJAK -> 7;
            case 499 -> 4;
            case 498 -> 7;
            ...
            case 2025, 2028, 963, 965 -> 7;
            case 3127 -> 8;
            case 8030, 8031 -> 5;
            case 2205 -> 4;
            case Brother.AHRIM -> 6;
            case Brother.DHAROK -> 7;
            case Brother.GUTHAN -> 5;
            case Brother.KARIL -> 4;
            case Brother.TORAG -> 5;
            case Brother.VERAC -> 5;
            case 3167, 2558, 2559, 2560, 2561, 2215 -> 6;
            // saradomin gw boss
    ...
    private int getProjectileStartHeight(int npcType, int projectileId) {
        return switch (npcType) {
            ...
            case 2044, 1443 -> 60;
            case 3162 -> 0;
            case 3127, 3163, InfernoWaveData.JALTOK_JAD, 3164, 3167, 3174 -> 110;
            default -> 43;
        };
    }
...
//        System.out.println("Took [" + (System.currentTimeMillis() - start) + "] MS to cycle " + (maxNPCs - 1) + " npcs");
    ...
    public void followTick(NPC npc, Player player, double distance, int followDistance, boolean run) {
     ...
     */
    public void attackPlayer(Player player, NPC npc) {
        if (Objects.isNull(npc)) return;
        ...
        if (npc.getCombatMethod() != null && npc.getNpcStats().scripts.combat_ != null) {
            npc.setCombatMethod(npc.getCombatMethod());
        }

        var def = NpcDef.forId(npc.getNpcId());
        ...
        if (!npc.getPosition().inMulti() && npc.underAttackBy > 0 && npc.underAttackBy != player.getIndex()) {
            npc.setPlayerAttackingIndex(0);
            npc.facePlayer(0);
            npc.underAttack = false;
            npc.randomWalk = true;
        ...
        if (!npc.getPosition().inMulti() && ((player.underAttackByPlayer > 0 && player.underAttackByNpc != npc.getIndex()) || (player.underAttackByNpc > 0 && player.underAttackByNpc != npc.getIndex()))) {
            npc.setPlayerAttackingIndex(0);
            npc.facePlayer(0);
            npc.underAttack = false;
            npc.randomWalk = true;
        ...
        switch (npc.getNpcId()) {
            case 8062:
                ...
                npc.gfx100(427);
                player.appendDamage(npc, 80, Hitmark.HIT);
                npc.setDead(true);
                break;
            case 1739:
            case 7413:
            ...
            case 6602:
                ...
                break;
            case Npcs.ABYSSAL_SIRE:
                boolean hasDistance = npc.getDistance(player.getX(), player.getY()) <= ((double) distanceRequired(npc)) + (npc.getSize() > 1 ? 0.5 : 0.0);
                if (!hasDistance) {
        ...
        if (npc.attackTimer == 0 && player.withinDistance(npc)) {
            if (npc.combatMethod instanceof CommonCombatMethod ccm) {
                npc.attackTimer = ccm.getAttackSpeed(npc);
                ccm.set(npc, player);
                ccm.prepareAttack(npc, player);
                return;
            }
        }
        ...
        if (ignoresProjectile(npc)) {
            if (distance < 10) {
                ...
                LoadSpell.loadSpell(player, npc);
        ...
        if (hasDistance) {
            /*if (npc.getAttackType() == CombatType.MELEE) {// this fixex atking tru walls
                if (!PathChecker.raycast(npc, player, false)) {
                    return;
                }
            }*/
            ...
            if (projectileClipping) {
                if (npc.getAttackType() == null || npc.getAttackType() == CombatType.MAGE || npc.getAttackType() == CombatType.RANGE) {
                    ...
                    if (!PathChecker.raycast(npc, player, true)
            ...
            if (player.respawnTimer <= 0) {
                ...
                if (npc.getNpcId() == Npcs.CERBERUS && Cerberus.cerberusSpecials(npc, player)) {
                    return;
                }
                npc.hitDelayTimer = getHitDelay(npc);
                ...
                player.singleCombatDelay2 = System.currentTimeMillis();
                startAnimationHighPriority(getAttackEmote(npc), npc);
                //if (player.getOpenInterface() != 39000) {
                // advanced options interface
                //player.getPA().removeAllWindows();
                //}
                if (npc.getAttackType() == CombatType.DRAGON_FIRE) {
                    npc.hitDelayTimer += 2;
                    player.getCombatItems().absorbDragonfireDamage();
                }
                if (multiAttacks(npc)) {
                    startAnimationHighPriority(getAttackEmote(npc), npc);
                    multiAttackGfx(npc);
                ...
                if (npc.projectileId > 0) {
                    if (npc.getNpcId() == 7706) {
                        NPC glyph = getNpc(7707, player.getHeight());
                        if (glyph == null) {
                            return;
                        }
                    }
                    ...
                    int centerX = nX + npc.getSize() / 2;
                    int centerY = nY + npc.getSize() / 2;
                    player.getPA().createPlayersProjectile(centerX, centerY, offX, offY, 50, getProjectileSpeed(npc), npc.projectileId, getProjectileStartHeight(npc.getNpcId(), npc.projectileId), getProjectileEndHeight(npc.getNpcId(), npc.projectileId), -player.getIndex() - 1, 65, npc.getProjectileDelay());
                ...
                if (npc.getNpcId() == InfernoWaveData.JAL_IMKOT && npc.getDistance(player.getX(), player.getY()) > 2 && Misc.random(0, 3) == 0) {
                    CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
                        ...
                        public void execute(CycleEventContainer container) {
                            ...
                            if (ticks == 2) {
                                npc.teleport(player.getX(), player.getY(), player.heightLevel);
                            }
                            if (ticks == 4) {
