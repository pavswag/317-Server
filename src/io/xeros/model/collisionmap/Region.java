...
public class Region {
    ...
    private static final String CUSTOM_MAPS_DIR = "./mapdata/custom_maps/";
    ...
    public void flagDecoration(int x, int y, int z) {
        addClipping(x, y, z, 0x40000);
    }

    public void clip(int objectId, int x, int y, int height, int type, int direction) {
        ObjectDef def = ObjectDef.getObjectDef(objectId);
        if (def == null) return;
        int xLength, yLength;
        if (direction == 1 || direction == 3) {
        ...
        if (type == 22) {
            ...
            if (def.aBoolean767) {
                addClipping(x, y, height, xLength, yLength, def.aBoolean757, false);
                if (def.aBoolean757) addClipping(x, y, height, xLength, yLength, true, true);
            }
        } else if (type >= 0 && type <= 3) {
            if (def.aBoolean767) {
                addVariableClipping(x, y, height, type, direction, def.aBoolean757, false);
                if (def.aBoolean757) addVariableClipping(x, y, height, type, direction, true, true);
            }
        }
    }
    ...
    public void addClipping(int x, int y, int z, int xLength, int yLength, boolean solid, boolean projectile) {
        ...
        for (int a = x; a < x + xLength; a++) {
            for (int b = y; b < y + yLength; b++) region.addClipping(a, b, z, mask, projectile);
        }
    }
...
//                            System.out.println("loaded region " +regionId);
        ...
        while ((incr = str1.readUnsignedIntSmartShortCompat()) != 0) {
            ...
            while ((incr2 = str1.get_unsignedsmart_byteorshort()) != 0) {
                ...
                if (height >= 0 && height <= 3) {
                    ...
                    region.clip(objectId, x, y, height, type, direction);
    ...
    public void addVariableClipping(int x, int y, int z, int type, int direction, boolean solid, boolean projectile) {
        if (type == 0) {
            if (direction == 0) {
