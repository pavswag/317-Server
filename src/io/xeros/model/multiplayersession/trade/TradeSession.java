...
/*				PlayerLogging.write(LogType.TRADE_LOG, player,
    ...
    public void give() {
        ...
        for (Player player : items.keySet()) {
            ...
            final ItemTradeLog e = new ItemTradeLog(player, other.getLoginName(), items.get(player), items.get(other));
            final ItemTradeLog o = new ItemTradeLog(other, player.getLoginName(), items.get(other), items.get(player));
            Server.getLogging().write(e);
            Server.getLogging().write(o);
            final String message =
                    "User1: [" + player.getDisplayName() + "] \n" +
                            "Items1: " + items.get(player) + "\n" +
                            "User2: [" + other.getDisplayName() + "] \n" +
                            "Items2: " + items.get(other);

            Discord.sendEmbeddedServerLog(
                    Discord.TRADE_CHANNEL,
                    Color.BLUE,
                    "Trade Logs",
                    message,
                    player.getLoginName(),
                    "User1 IP: " + player.getIpAddress() + " \n" +
                            "User2 IP: " + other.getIpAddress()
            );
