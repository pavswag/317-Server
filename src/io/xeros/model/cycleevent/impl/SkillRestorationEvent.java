...
import static io.xeros.content.skills.Skill.*;
...
public class SkillRestorationEvent extends Event<Player> {
	...
	@Override
	public void execute() {
		if (attachment.isDead || attachment.getHealth().getCurrentHealth() <= 0) return;
		...
		for (int index = 0; index < attachment.playerLevel.length; index++) {
			...
			if ((index == 0 || index == 1 || index == 2) && attachment.hasDivineCombatBoost) continue;
			if ((index == DEFENCE.getId()) && attachment.hasDivineDefenceBoost) continue;
			if ((index == RANGED.getId()) && attachment.hasDivineRangeBoost) continue;
			if ((index == MAGIC.getId()) && attachment.hasDivineMagicBoost) continue;
			if ((index == 0 || index == 1 || index == 2 || index == 4 || index == 6) && attachment.hasOverloadBoost)
                continue;
