...
public class Bank {
	...
	public int getBankCapacity() {
		return Configuration.BANK_CAPACITY + (int) (Right.DONATOR_SET.stream().filter(
				right -> player.getRights().getSet().stream().anyMatch(
						right2 -> right2.isOrInherits(player))).count() * 150);
	}
	...
	public void sendValue() {
		...
		player.getPA().sendString(59117, "Bank value: <col=FF5500>" + Misc.formatCoins(coins) + " Coins, " + Misc.formatCoins(foundry) +" Upgrade Points.");
	}
