...
public class ItemDef {
    ...
    @Getter
    private static final Int2ObjectMap<ItemDef> definitions = new Int2ObjectOpenHashMap<>();
    ...
    public static void load() throws Exception {
        ...
        List<ItemDef> list = JsonUtil.fromYaml("C:/Users/"+ Configuration.SAVES+"/Dropbox/etc/cfg/item/item_definitions.yaml", new TypeReference<>() {});

        for (var entry : list) definitions.put(entry.id, entry);

        for (var def : definitions.int2ObjectEntrySet()) {
            Preconditions.checkState(def.getValue() != null, "No parent definition for id={}, parent={}", def.getIntKey(), def.getValue());
            definitions.put(def.getIntKey(), def.getValue());
        }
