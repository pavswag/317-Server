...
public enum PrestigePerks {
    ...
    ATTUNE_PERKS(4, 3, 18943, 18944, 2409, 1000, 30_000_000),//10 % chance to save item / Upgrade Points when upgrading
    ...
    public static void HandleButtons(Player player, PrestigePerks value) {
        ...
        player.start(new DialogueBuilder(player).option("This relic costs " + value.madCost + " Upgrade Points & " + value.cost + " prestige pooints!",
                new DialogueOption("Buy relic", p -> {
                    if (value.madCost > player.foundryPoints) {
                        p.sendMessage("You need " + Misc.formatCoins(value.madCost) + " Upgrade Points to purchase this perk!");
