...
public class WorldEventContainer {
    ...
    public static final List<WorldEvent> WORLD_EVENT_LIST =
            Collections.unmodifiableList(Lists.newArrayList(new TournamentWorldEvent(), new HesporiWorldEvent(),
    ...
    private void next() {
        ...
        if (random == 5) {
            PlayerHandler.getPlayers().forEach(p -> p.forcedChat("Oo some Misc crystals!"));
            PlayerHandler.getPlayers().forEach(p ->  p.getItems().addItemUnderAnyCircumstance(33135, Misc.random(5,55)));
        }
        scheduleNext();
    }
