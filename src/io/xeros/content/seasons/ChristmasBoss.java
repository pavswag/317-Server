...
import static io.xeros.content.seasons.ChristmasTsnumaiPaths.*;
...
public class ChristmasBoss {
    ...
    public static void initBoss() {
        ...
        evilSnowman = NPCSpawning.spawnNpc(2847, 2916, 3941, 0, 1, 30, npcStats);
           ...
           // PlayerHandler.executeGlobalMessage("<img=28>[@red@C@gre@H@red@R@gre@I@red@S@gre@T@red@M@gre@A@red@S@bla@]<img=28> @pur@ The Evil Yeti Has died!");
                ...
                if (map.containsKey(s) && map.get(s) > 1) {
                    for (Player player : PlayerHandler.getPlayers()) {
                        if (player.getUUID().equalsIgnoreCase(s)) {
                            Discord.writeServerSyncMessage("[Evil YETI] " + player.getDisplayName() + " has tried to take more than 2 account's there!");
                        }
                    }
                }
            }
            ...
            damageCount.forEach((player, integer) -> {
                if (integer > 100 && map.containsKey(player.getUUID())) {
                    ...
                    Server.getDropManager().create(player, npc, new Location3D(player.getX(), player.getY(), player.getHeight()), amountOfDrops, 2847);
