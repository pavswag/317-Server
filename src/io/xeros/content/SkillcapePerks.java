...
public enum SkillcapePerks {
	...
	public static boolean isWearingMaxCape(Player player) {
		var capeDef = ItemDef.forId(player.playerEquipment[Player.playerCape]);
		if (Objects.nonNull(capeDef))
			return capeDef.getName().toLowerCase().contains("max cape");

		return player.getItems().isWearingAnyItem(Items.COMPLETIONIST_CAPE) ||
				player.getItems().isWearingAnyItem(23859) ||
