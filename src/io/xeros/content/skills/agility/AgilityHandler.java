...
public class AgilityHandler {
    public static int[] gracefulIds = { 11850, 11852, 11854, 11856, 11858, 11860,
    ...
    public void lapFinished(Player c, int progress, int experience, int petChance) {
        if (agilityProgress[progress]) {
            ...
            int pieces = 1;
            for (int k : gracefulIds) {
                if (c.getItems().isWearingItem(k)) {
                    pieces += 6;
                }
            }
            c.sendMessage("You received some XP for completing the track!");
            c.getPlayerAssistant().addSkillXPMultiplied(experience * xpBonus(c), 16, true);
