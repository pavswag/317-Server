...
public class Solak {
    ...
    public static void checkCore(NPC corp) {
        ...
        if (corp.getHealth().getCurrentHealth() < 2000) {
            NPCSpawning.spawnNpc(Npcs.ADAMANT_DRAGON, 2156, 3428, 0, 0, 20);
    ...
    public static void healWhenNoPlayers(NPC corp) {
        if (!corp.getHealth().isMaximum()) {
            if (Boundary.getPlayersInBoundary(Boundary.SOLAK) == 0) {
                corp.getHealth().reset();
                NPC core = NPCHandler.getNpc(Npcs.ADAMANT_DRAGON);
                if (core != null)
    ...
    public static void handleRewards(NPC npc) {
        ...
        damageCount.forEach((player, integer) -> {
            if (integer > 1 && Boundary.isIn(player, Boundary.SOLAK) && map.containsKey(player.getUUID())) {
                int amountOfDrops = 1;
                if (NPCDeath.isDoubleDrops()) {
                    amountOfDrops++;
                }
                Server.getDropManager().create(player, npc, new Location3D(player.getX(), player.getY(), player.getHeight()), amountOfDrops, 1028);
