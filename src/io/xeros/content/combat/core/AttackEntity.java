...
public class AttackEntity {
    ...
    public void attackEntity(Entity targetEntity) {
        ...
        if (attacker.isInvisible() && !Right.STAFF_MANAGER.isOrInherits(attacker)) {
        ...
        if (targetEntity.isPlayer()) {
            ...
            if (!Boundary.isIn(attacker, Boundary.DUEL_ARENA) && !TourneyManager.getSingleton().isInArena(attacker) && !WGManager.getSingleton().isInArena(attacker)) {
                if (!attacker.attackedPlayers.contains(attacker.playerAttackingIndex) && !PlayerHandler.players[attacker.playerAttackingIndex].attackedPlayers.contains(attacker.getIndex()) && !attacker.isSkulled) {
                    if (!targetPlayer.getMode().equals(Mode.forType(ModeType.WILDYMAN)) && !targetPlayer.getMode().equals(Mode.forType(ModeType.HARDCORE_WILDYMAN))) {
            ...
            if (getCombatType() == CombatType.MAGE) {
                ...
                if (getCombatType() == CombatType.RANGE) {
                    ...
                    if (attacker.playerEquipment[Player.playerWeapon] >= 4212 && attacker.playerEquipment[Player.playerWeapon] <= 4223
                            ...
                            || attacker.playerEquipment[Player.playerWeapon] == 29000
    ...
    private boolean determineWeaponStyle() {
        ...
        if (!attacker.usingMagic) {
            ...
            if (attacker.getItems().isWearingItem(28688)) {
