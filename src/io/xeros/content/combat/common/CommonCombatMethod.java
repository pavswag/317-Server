package io.xeros.content.combat.common;

import io.xeros.Server;
import io.xeros.content.combat.common.annotations.CombatScript;
...
public abstract class CommonCombatMethod implements CombatMethod {
    ...
    public void doFollowLogic() {
        if (entity instanceof NPC npc && target instanceof Player player) {
            npc.randomWalk = false;
            npc.facePlayer(player.getIndex());
            if (!withinDistance()) {
                NPCDumbPathFinder.follow(npc, player);
            }
            if (!PathChecker.hasLineOfSight(entity, target)) {
                NPCDumbPathFinder.follow(npc, player);
            }
        }
    }
