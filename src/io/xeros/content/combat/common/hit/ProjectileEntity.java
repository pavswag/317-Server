package io.xeros.content.combat.common.hit;
...
public class ProjectileEntity {
    ...
    public ProjectileEntity(Entity source, Entity victim, int projectileId, int startDuration, int travelTime, int startHeight, int endHeight, int curve, int creatorSize, int stepMultiplier) {
        this(source.getCenterPosition(), victim.getCenterPosition(), getLockon(victim), projectileId, travelTime, startDuration, startHeight, endHeight, curve, creatorSize, 64, stepMultiplier);
    }

    public ProjectileEntity(Entity source, Entity victim, int projectileId, int delay, int speed, int startHeight, int endHeight, int creatorSize, int stepMultiplier) {
        this(source.getCenterPosition(), victim.getCenterPosition(), getLockon(victim), projectileId, speed, delay, startHeight, endHeight, 16, creatorSize, 64, stepMultiplier);
    }
    ...
    public ProjectileEntity(Entity source, Entity victim, int projectileId, int delay, int speed, int startHeight, int endHeight, int curve, int creatorSize, int startDistanceOffset, int stepMultiplier) {
        this(source.getCenterPosition(), victim.getCenterPosition(), victim.getIndex(), projectileId, speed, delay, startHeight, endHeight, curve, creatorSize, startDistanceOffset, stepMultiplier);
    }
