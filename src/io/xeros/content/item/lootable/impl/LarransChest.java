...
public class LarransChest implements Lootable {
	...
	public void roll(Player c) {
		if (c.getItems().playerHasItem(KEY)) {
			...
			if (random == 0) {
				PlayerHandler.executeGlobalMessage("@pur@" + c.getDisplayNameFormatted() + " received a drop: " + ItemDef.forId(reward.getId()).getName() + " x " + reward.getAmount() + " from Larran's chest.");
			}
